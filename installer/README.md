...
- 交互安装允许联网安装 .NET Framework 4.8。3010、1641 始终要求重启，即使 Release 注册表已经更新；同一次安装不能绕过此状态。
- 返回 0 后仍未检测到运行时，作为安装失败处理，不推断为成功或需要重启。
- 静默模式缺少运行时时直接失败，不下载、不询问。先由部署系统安装 .NET，再运行 AccuX 安装包。
- 准备阶段失败返回 7，需要重启返回 8；COM 注册错误触发安装撤销并返回 4。这些返回值已在 Inno Setup 6.7.3 下验证。

```powershell
.\AccuXSetup-1.0.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /SP- /LOG="AccuX-install.log"
```
