...
一键舍入、金额折合、金额大写、选区求和和底色标记支持同一工作表内按 Ctrl 选择多个不连续区域。

- 每个区域分别与 `UsedRange` 求交，重叠单元格只处理一次，隐藏行/列仍跳过。
- 公式、数值和格式在原位置处理，不修改区域之间未选中的单元格。
- 全部区域先完成写前校验，再开始写回；运行中 COM 写入异常不提供事务回滚。
- 单元格上限按裁剪、去重后的总数计算；原始区域和去重碎片最多各 4096 个。
- 区域比较仍要求每一侧为单个连续区域；跨工作表多选不在本功能范围内。

真实 Excel 集成验证（先完成 Debug 构建）：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test-multi-area.ps1
```

脚本新建临时工作簿并在结束时不保存关闭，检查重叠去重、公式位置、未选中单元格、整体写前校验、筛选隐藏和 UsedRange 裁剪。多区域行为已在本机 Excel 验证，WPS 仍需实机验证。
...
> UsedRange 限定先于批量读取与阈值检查；`RangeTarget` 的地址、行列数和单元格数按 Selection 与原生 `UsedRange` 的交集计算。隐藏行/列是与数据类型独立的公共规则：交集范围内的隐藏单元格不会进入四个财务功能的处理；选区求和只复制格式化后的合计数字。
